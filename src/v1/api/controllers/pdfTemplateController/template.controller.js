...
import { addTemplate, getAllTemplates, getProductTemplateId, getTemplateId, removeTemplate, updateTemplateById } from "../../formatters/pdfTemplateFormatter/template.formatter.js";
...
export async function getTempByproduct(req, res) {
 try {
 const { status, message, data } = await getProductTemplateId(req)
 return status ? success(res, message, data) : badRequest(res, message)
 } catch (error) {
 return unknownError(res, error.message)
 }
