import { addField, deleteFieldById, getAllByReportType, getAllFields, getFieldById, updateFieldById } from "../../helper/reportHelper/inputField.helper.js"
...
export async function getAllnputByOrganizationId(req, res) {
    try {
...
export async function getAllnputByreportId(req, res) {
    try {
        const { status, message, data } = await getAllByReportType(req)
        return status ? success(res, message, data) : badRequest(res, message)
    } catch (error) {
        return unknownError(res, error.message)
    }
