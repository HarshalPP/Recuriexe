...
import {  updatePurchaseRequest, getAllPurchases } from "../../helper/expense/purchaseRequest.helper.js"
...
export async function updatePurchase(req, res) {
    try {
        const { status, message, data } = await updatePurchaseRequest(req?.body?.id,req)
...
export async function getAllPurchase(req, res) {
    try {
        const { status, message, data } = await getAllPurchases()
        return status ? success(res, message, data) : badRequest(res, message)
    } catch (error) {
        return unknownError(res, error.message)
    }
