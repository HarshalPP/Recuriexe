...
import templateModel from "../../models/pdfTemplateModel/pdfTemplate.model.js";
// import { storeBackupLog } from "./log.helper.js";
...
export async function addTemplate(requestsObject) {
 try {
 const newData = await templateModel.create({serviceId:requestsObject.user.serviceId,...requestsObject.body});
...
export async function removeTemplate(tempId) {
 ...
 await templateModel.findByIdAndDelete(varId);
...
export async function getProductTemplateId(req) {
 try {
 const varData = await templateModel.find({userProductId:req.query.productId,serviceId:req.user.serviceId})
 return returnFormatter(true, "Data fetched succesfully",varData)
 } catch (error) {
 return returnFormatter(false, error.message)
 }
...
export async function getAllTemplates(requestsObject) {
 try {
 const varData = await templateModel.find({ serviceId: requestsObject.user.serviceId })
 .populate({
 path: "userProductId",
 model: "userProduct",
