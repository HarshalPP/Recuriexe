...
import variableModel from "../../models/pdfTemplateModel/variableSetUp.model.js";
...
// import initFieldModel from "../models/initFields.model.js";
...
export async function addVariable(requestsObject) {
  try {
    const newData = await variableModel.create({ organizationId: requestsObject.employee.organizationId, variableName: `{${requestsObject.body.variableName}}` })
...
export async function getAllVarible(requestsObject) {
  try {
    const varData = await variableModel.find({ organizationId: requestsObject.employee.organizationId });
    return returnFormatter(true, "Data fetched succesfully", varData)
  } catch (error) {
    return returnFormatter(false, error.message)
  }
