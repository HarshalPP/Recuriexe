...
async function bodEodAdd(req, res) {
  try {
    ...
    const {assignBy, employeeId, task, description } = req.body;

    if (!Array.isArray(employeeId) || employeeId.length === 0) {
      return badRequest(res, "Please provide employeeId in an array format");
    }

    if (!task || task.trim() === "") {
      return badRequest(res, "Please provide a valid task");
    }

    const startDate = moment().tz("Asia/Kolkata").format("YYYY-MM-DDThh:mm:ss A");

    // Validate each employeeId
    ...
    // Prepare BOD/EOD entries for all valid employees
    const bodEodEntries = validEmployees.map((employee) => ({
