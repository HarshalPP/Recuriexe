...
async function sanctionLetterPdf(allPerameters,logo,partnerName) {
  ...
  function addFooter() {
    if( partnerName == "GROW MONEY CAPITAL PVT LTD"){
      ...
      doc
        ...
        .fontSize(6.3)
        .fillColor("#000000")
        .text("GROW MONEY CAPITAL PVT LTD, 401, New Delhi House, 27 Barakhamba Road, Connaught Place, New Delhi-110001", {
          align: "center",
        });
      doc
        .font(fontBold)
        .fontSize(6.3)
        .fillColor("#000000")
        .text("CIN: U74899DL1995PTC069216", { align: "center" });
      doc
        .font(fontBold)
        .fontSize(6.3)
        .fillColor("#000000")
        .text("Phone: +011-43580437, 011-40045955 | Email:  support@growmoneycapital.com", {
          align: "center",
        });
  ...
  function drawTable(tableData) {
    ...
    tableData.forEach((row, rowIndex) => {
      ...
      if ([12, 14, 17, 19].includes(rowIndex)) {
        valueRowHeight = 38.5;
      }
  ...
  const loanTableData = [
    ...
    { field1: "Current GROW Money Reference Rate", value1: `${allPerameters.benchmarkInterest}% per annum` },
    {
      field1: "Applicable Floating Rate of Interest (Current URR +/- Margin)",
      value1: `${allPerameters.benchmarkInterest}%+ ${allPerameters.SpreadInterestRate}% = ${allPerameters.interestRate}% per annum`,
    },
    {
      field1: "Installment Amount/ Graded Installments",
      value1: `Rs ${allPerameters.emiAmount} for a period of ${allPerameters.tenureinMonths} months`,
    },
    // {
    ...
    {
      field1: `EMI/ Installment date (Exact dates of repayment depends on the disbursement date and the same shall be mentioned in the loan agreement and repayment schedule whicand repayment schedule which shall be shared with the Borrower post disbursement)`,
...
GROW MONEY CAPITAL PVT LTD`,
  ...
  doc
    ...
    .fontSize(9)
    .fillColor("black")
    .text("Call us at: +011-43580437, 011-40045955")
    .fillColor("blue")
  ...
  doc
    ...
    .fontSize(8)
    .fillColor("black")
    .text("   Bank Account Number - 18455600001636", { align: "center"}).moveDown(1)
    .text("   Bank Account Name - GROW MONEY CAPITAL PVT LTD", { align: "center"}).moveDown(1)
    .text("   Bank Name - FEDERAL BANK LIMITED", { align: "center"}).moveDown(1)
    .text("   Branch Name - New Delhi", { align: "center"}).moveDown(1)
    .text("   IFSC Code - FDRL0001845", { align: "center"})
    .moveDown(1);
...
// const generateSanctionLetterPdf = async(res,customerId) =>{
  ...
  //       const branchName = branchData.name; 
    ...
    const emiAmounts  = finalsanctionDetails?.emiAmount;
     ...
     const epiround =disbuDetail?.kfsDetails?.epi
     const roundEpi = Math.round(epiround);
    ...
    const allPerameters = {
      ...
      const uploadResponse = await uploadPDFToBucket(pdfPath, `growSanctionLetter${Date.now()}.pdf`);
      const url = uploadResponse.url
        console.log(url,"url")
        await finalsanctionModel.findOneAndUpdate(
          { customerId }, // Query to find the specific customer document
          {
            $set: { "growMoneyPdfUrls.growSanctionLetterPdf": url } // Dot notation for nested update
          },
