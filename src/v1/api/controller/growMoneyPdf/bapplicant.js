...
async function growpdf(allPerameters,logo,partnerName) {
  ...
  function addLogo(doc) {
    if (fs.existsSync(logo)) {
      doc.image(logo, 400, 50, { fit: [150, 50], align: "right", valign: "bottom" });
    } else {
...
const imagelogo =path.join(__dirname, `../../../../..${allPerameters.appImage}`);
  ...
  const declarations = [
    ...
    `The executive collection of the application/documents has informed me/us of the rate of interest and approach for gradation of risk and rational\nof charging different rates of interest to different categories of borrowers, the particulars whereof have specified in the Loan Application form.`,
    ...
    `The credit decision is based on the credit model which includes factors like credit history, repayment track record, banking habit, business stability & cash low analysis which is assessed through a combination of personal discussion and documentation`,
  ];
  ...
  const DeclarationDetails = [
    ...
    `I/We acknowledge and understand that the application/processing fees collected from me/us by Fin Coopers Capital Pvt Ltd, is for reviewing the loan application as per its own parameters and its not refundable to me/us under any circumstances whatsoever, irrespective of whether Fin\nCoopers Capital Pvt Ltd sanction this loan application of mine or not. No cash has been given by me/us to any person for whatsoever reason\nrelated to the loan application.`,
...
async function growApplicantPdf(customerId,selections = "ac",logo,partnerName ) {
  try {
 ...
 console.log(selections,"selections"); // "acg"
    ...
    const creditPdDetails = await creditPdModel.findOne({customerId})
      ...
      let allParameters = {
        ...
        coAPPDob2 : coApplicantDetails?.[1]?.dob || "NA",//page no.1
        ...
        coAPPAdhar2 : coApplicantDetails?.[1]?.aadharNo || "NA",//page no.1
...
module.exports = { growpdf, growApplicantPdf };
