...
async function growpdf(allPerameters) {
 ...
 /// make a function Singnature //
  ...
  const applicantDetails = [

    {
      ...
      key: "Father/Spouse Name", value: `${allPerameters.appFather}`
    }, {
  ...
  const GuarnatorDetails = [
    {
      ...
      key: "Mobile No.2",
      value: `${allPerameters.guaMobileNo2}`
    }, {
      key: "Email ID",
...
doc.moveDown(5);
  ...
  function createAadhaarConsentPDF(executants) {
    ...
    const place = `${allPerameters.appCityName}`;
  ...
  try {
    ...
    const creditPdDetails = await creditPdModel.findOne({customerId})
      ...
      const allPerameters = {
        ...
        appDOB : applicantDetails?.dob || "NA",//page no.1
        ...
        appAdhar : applicantDetails?.aadharNo || "NA",//page no.1
        AppVoterId : applicantDetails?.voterIdNo || "NA",//page no.1

        //   communicationAddress
        appadharadress : address || "NA",
        appCityName : applicantDetails?.permanentAddress?.city|| "NA",
        appdistrict : applicantDetails?.permanentAddress?.district|| "NA",
        AppPin : applicantDetails?.permanentAddress?.pinCode|| "NA",
        AppState : applicantDetails?.permanentAddress?.state|| "NA",
        AppYearsAtCureentAdress : applicantDetails?.permanentAddress?.years || "NA",
        ...
        coAPPDob : coApplicantDetails?.[0]?.dob || "NA",//page no.1
        coAppGender : coApplicantDetails?.[0]?.gender || "NA",//page no.1
        coAppMarritalStatus : coApplicantDetails?.[0]?.maritalStatus || "NA",//page no.1
        coAppPan : coApplicantDetails?.[0]?.panNo || "NA",//page no.1
        coAPPAdhar : coApplicantDetails?.[0]?.aadharNo || "NA",//page no.1
        coAppvoterId : coApplicantDetails?.[0]?.voterIdNo || "NA",//page no.1

        //   communicationAddress
        coAppAdharAdress : coborroweraddress || "NA",
        coAppcity : coApplicantDetails?.[0]?.permanentAddress?.city|| "NA",
        coAppdistrict : coApplicantDetails?.permanentAddress?.[0]?.district|| "NA",
        coAppPIN : coApplicantDetails?.[0]?.permanentAddress?.pinCode|| "NA",
        coAppState : coApplicantDetails?.[0]?.permanentAddress?.state|| "NA",
        coAppcurentAdress : coApplicantDetails?.[0]?.permanentAddress?.years || "NA",
      ...
      guaType : creditPdDetails?.guarantor?.[0]?.guarantorType || "NA",
      ...
      guaDob : guarantorDetails?.dob || "NA",//page no.1
      guaGender : guarantorDetails?.gender || "NA",//page no.1
      guaMaritialStatus : guarantorDetails?.maritalStatus || "NA",//page no.1
      guaPan : guarantorDetails?.panNo || "NA",//page no.1
      guaAdhar : guarantorDetails?.aadharNo || "NA",//page no.1
      guaVoterId : guarantorDetails?.voterIdNo || "NA",//page no.1

        //   communicationAddress
        guaAdressAdhar :guarantorDetails?.permanentAddress?.addressLine1|| "NA",
        guaCity : guarantorDetails?.permanentAddress?.city|| "NA",
        guaDist : guarantorDetails?.permanentAddress?.district|| "NA",
        guaPin : guarantorDetails?.permanentAddress?.pinCode|| "NA",
        guaState : guarantorDetails?.permanentAddress?.state|| "NA",
        guaYearsCurrentAddress : guarantorDetails?.permanentAddress?.years || "NA",
    ...
    const pdfPath = await growpdf(allPerameters);
