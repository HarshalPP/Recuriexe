...
async function growpdf(allPerameters,skipPages) {
  ...
  function addFooter1(doc) {
    ...
    doc.font(fontBold).fontSize(6.3).fillColor("#324e98").text("Namdev Finvest Private Limited", pageWidth, pageHeight - 80, { align: "center" });
    doc.font(fontBold).fontSize(6.3).fillColor("#000000").text("Registered Ofice Address: S1-S7-S8 Second Floor, HDFC Bank Building Shree Nath Plaza,Ajmer Rd, Neer Sagar Market", { align: "center", });
    doc.font(fontBold).fontSize(6.3).fillColor("#000000").text("Bhakrota,Jaipur,Rajasthan 302026", { align: "center" });
    doc.font(fontBold).fontSize(6.3).fillColor("#000000").text("CIN : U652921RJ1997PTC047090,EMAIL ID:info@namfin.in CONTACT:-01412250026", { align: "center",link: "tel:01412250026",link: "mailto:info@namfin.in", // Make it clickable
  });
  ...
  // const pdfFilename = `namDevApplicantConditions.pdf`;
  ...
  function drawTable(sectionTitle, data) {
    ...
    const specialRowColor = "#00BFFF"; // Light blue background color#00BFFF.
...
following information and have accordingly filled up the aforesaid form.`);
 ...
 /// make a function Singnature //
  ...
  function createStyledTable(doc, headers, tableData, isHeaderBoxed = false) {
    ...
    if (isHeaderBoxed) {
      ...
      doc
        ...
        .fillColor('#00BFFF') // Blue background for the header
      ...
      doc
        .fillColor('black') // Black text color
        .font('Helvetica-Bold')
        .fontSize(8)
  ...
  const tableData1 = [
    ...
    { col1: 'Loan Against Property/Shed Purchase', col2: 'AGRI MICRO LAP', col3: 'Others', col4: 'NA' },
  ];
  
  const headers2 = ['Product Program Details'];
  const tableData2 = [
    { col1: 'Industry Type', col2: 'NA', col3: 'Sub Industry Type', col4: 'NA' },
    { col1: 'Product Type', col2: 'AGRI MICRO LAP', col3: 'Secured/Unsecured', col4: 'SECURED' },
...
//     return Buffer.from(response.data, 'binary'); // Return the image as a buffer
//   } catch (error) {
...
// async function drawTablenew(sectionTitle, data, imagePath) {
//   // const imageBuffer = await fetchImage(imagePath);  // Fetch image buffer

//   // console.log("imageBugger" ,  imageBuffer)
...
//   doc.font("Helvetica-Bold").fontSize(10).fillColor("black").text(sectionTitle, startX + 5, startY + 5);
...
//     return response.data; // Image as buffer return karega
//   } catch (error) {
...
// async function drawTablenew(doc, sectionTitle, data, imagePath) {
//   doc.moveDown(1);
//   const titleHeight = 20;
//   const titleX = 48;
//   const titleWidth = doc.page.width - 2 * titleX;
...
//   const startX = titleX;
//   let startY = doc.y + titleHeight; // Ensure y position updates properly
//   const columnWidthsFirst5 = [125, 275];
//   const columnWidths = [125, 125, 125, 125];
//   const imageWidth = 100;
//   const imageSpanRows = 5;
...
//   // **Title Section**
//   doc.rect(startX, startY, titleWidth, titleHeight).fill("#00BFFF").strokeColor("black").lineWidth(1).stroke();
//   doc.font("Helvetica-Bold").fontSize(10).fillColor("black").text(sectionTitle, startX + 5, startY + 5);
//   startY += titleHeight;

//   // **Fetch Image Buffer (Wait for Image Load)**
...
//         if (imageBuffer) {
//           doc.image(imageBuffer, startX + columnWidthsFirst5[0] + columnWidthsFirst5[1] + 5, rowY + 5, {
//             fit: [imageWidth - 10, imageHeight - 10],
//           });
//         } else {
//           doc.font("Helvetica-Bold").fontSize(10).fillColor("#ff0000")
...
//   doc.moveDown(2); // Ensure spacing after table
// }
...
//     if (index < 5) {
//       rowHeight = Math.max(
//         doc.heightOfString(row.key || "", { width: columnWidthsFirst5[0] - 10 }),
//         doc.heightOfString(row.value || "", { width: columnWidthsFirst5[1] - 10 })
//       ) + 10;
//     } else {
//       rowHeight = Math.max(
//         doc.heightOfString(row.key1 || "", { width: columnWidths[0] - 10 }),
//         doc.heightOfString(row.value1 || "", { width: columnWidths[0] - 10 }),
//         doc.heightOfString(row.key2 || "", { width: columnWidths[2] - 10 }),
//         doc.heightOfString(row.value2 || "", { width: columnWidths[3] - 10 })
//       ) + 10;
//     }
...
//         if (imagePath.startsWith("http")) {
//           const imageBuffer = await fetchImageBuffer(imagePath);
//           if (imageBuffer) {
//             doc.image(imageBuffer, startX + columnWidthsFirst5[0] + columnWidthsFirst5[1] + 5, rowY + 5, {
//               fit: [imageWidth - 10, imageHeight - 10],
//             });
...
//   try {
//     const fileName = `temp_image_${Date.now()}.jpg`; // Temporary file name
//     const filePath = path.join(__dirname, fileName); // Full local file path
...
//     // Write the image stream to a local file
...
// Updated logic to handle live URL or local path
function drawTablenew(sectionTitle, data, imagePath) {
  doc.moveDown(1);
  const titleHeight = 20;
  const titleX = 48;
...
// async function drawTablenew(sectionTitle, data, imageUrl) {
//   doc.moveDown(1);
//   const titleHeight = 20;
//   const titleX = 48;
//   const titleWidth = doc.page.width - 2 * titleX;

//   console.log("Function Called: drawTablenew");
//   console.log("Section Title:", sectionTitle);
//   console.log("Applicant Details Data:", JSON.stringify(data, null, 2));
...
//       console.log("Fetching image from local path:", imageUrl);
//       imageBuffer = fs.readFileSync(imageUrl);
//       console.log("Fetching imageBuffer from local path:", imageBuffer);
...
// const imagePath = `${allPerameters.appImage}`;
...
const fetchImage = async (imageUrl) => {
  try {
      ...
      console.error("Error fetching image:", error);
      return null; // Agar error aaye toh image ko skip kar sake
  }
};
...
// const imagePath = "./uploads/applicant_photo.jpg";
// const imagePatha = path.join(__dirname, `../../../../..${allPerameters.co2Image}`);
...
function drawTablenew2(sectionTitle, data, imagePath) {
  ...
  // //console.log("Applicant Details Data:", gauranterDetailsData);
  // const imagePath = "./uploads/applicant_photo.jpg";
  // const imagePath2 = path.join(__dirname, `../../../../..${allPerameters.guaImage}`);
  ;
...
const CollateralsDetails = [
  ...
  ["Name of Registered Owner", `${allPerameters.propertyHolder}`, "Relationship with Borrower", `${allPerameters.relationWithborrow}`],
  ["Area (In sq.ft)", `${allPerameters.sreaInSqFt}`, "Age of Property (In years)", `${allPerameters.propertyaGE}`],
  { key: "Market Value as on Date", value: `${allPerameters.marketValue}` }
];
...
drawTableCollateral("Collaterals Details", CollateralsDetails);
  ...
  function drawTitletit(sectionTitle) {
    ...
    const titleBackgroundColor = "#00BFFF";  // Background color (blue)
...
2. I/We shall advise Namdev Finvest Private Limited (NFPL) in writing of any change in my/our residential or employment/ business address.
...
5. I/We authorise NFPL to make any enquiries regarding my/our application, including with other inance companies/registered credit bureau.
6. NFPL reserves the right to retain the photographs and documents submitted with this application and will not return the same to the applicant/s.If i/we wish to revoke this consent for sharing data that is available in the data base in confidential form, i/we may submit a return application to the nearest branch. Upon receipt of such application, the company will cease sharing the information with any third party, except for restrictive use and official purposes are allowed by regulatory authorities. The company shall retain the data for a period not exceeding 8 years, as mandatory by RBI guidelines.
7. I/We have read the application form/ brochures and am/are aware of all the terms and conditions of availing inance from NFPL.
8. I/We understand that the sanction of this loan is at the sole discretion of NFPL and upon my/our executing necessary 
security (ies) and other formalities as required by NFPL and no commitment has been given regarding the same.
9. I/We authorise NFPL to conduct such credit checks as it considers necessary in its sole discretion and also authorise
NFPL to release such or any other information in its records for the purpose of credit appraisal/sharing for any other
purpose. I/We further agree that my/our loan shall be governed by the rules of NFPL which may be in force from time to
time.
...
11. I/We am/are aware that NFPL does not accept any payment in cash. No payment in connection with the loan 
processing, sanction, disbursement, prepayment and repayment of loan shall be made to / in favour of any of
NFPL intermediaries or any third party (ies) in cash or bearer cheque or in any other manner whatsoever.
12. No discount/free gift or any other commitment whatsoever has been which is not documented in the loan
agreement by NFPL or any of its authorised representatives.
...
.fontSize(9)
.text(
`Heads of States/ Governments, senior politicians, senior government or judicial or military of oficers, senior executives of state-owned corporations and important 
Please tick Yes / No:
...
change in the money market conditions or on account of any other statutory or regulatory requirements or at NFPL 
discretion.NFPL reserves the right to review and amend the terms of the loan in such manner and to such extent as
it may deem it.
16. I/We hereby declare and confirm if any detail or declaration made by me/us, if found to be false, then NFPL will be entitled to revoke and/or recall the credit facility.
17. I/We hereby declare and conirm that any purchase by me/us of any insurance product is purely voluntary and is not
linked to availing of any credit facility from NFPL.
...
20. I/We have never been a defaulter with NFPL or any other inancial institution.
21. That if any discrepancy is found or observed from the information given above and the documents produced in support 
thereof, NFPL shall have the sole discretion to cancel the sanction at any stage and recall the loan if already disbursed
,in such an event, the processing fee shall be liable to be forfeited.
22. I/We permit NFPL to contact me/us with respect to the products and services being offered by NFPL or by any other
person (s) and further allow NFPL to cross sell the other products and services offered by such other person(s).
23. I/We further agree to receive SMS alerts/whatsapp/emails/letters etc. related to my/our application status and account
activities as well as product use messages that NFPL and/or its group companies will send, from time to time on my/our 
...
25. I/We further agree to comply with KYC norms of RBI and submit all relevant KYC data and information as may be requisitioned by NFPL from time to time.
26. I/We shall create security and/or furnish guarantee in favour of NFPL as may be required.
27. I hereby submit voluntarily at my own discretion, the physical copy of Aadhaar card/physical e-Aadhaar / masked
Aadhaar / ofline electronic Aadhaar xml as issued by UIDAI (Aadhaar), to NFPL for the purpose of establishing my
identity / address proof.
28. The consent and purpose of collecting Aadhaar has been explained to me in local language. NFPL has informed me
that my Aadhaar submitted to NFPL herewith shall not be used for any purpose other than mentioned above, or as per requirements of law.
`,
{ align: 'justify',  lineGap: 5 }
).moveDown();
  ...
  .text(`
    ...
    I/we understand that my KYC Record includes my/our KYC Records / Personal information such as my/our name, address, date of birth / PAN number etc.
    ...
    address. | also agree that the non-receipt of any such SMS/e-mail shall not make the NFPL liable for any
    ...
    Applicant's signature       Co-Applicant's signature         Co-Applicant's signature              Guarantor's signature`,
  ...
  doc.font('Helvetica')
  .fontSize(9)
  .text(`The NFPL's Sales Representative conirms he has: 
    ...
    (d) Informed me/us that the NFPL will not be liable for loss or delay in receipt of documents.
    (e) Informed me/us at incomplete / defective application will not be processed and the NFPL shall not be responsible in
    any manner for the resulting delay or otherwise. Notwithstanding the afore stated, the submission of loan application
    to the NFPL does not imply automatic approval by the NFPL and the NFPL will decide the quantum of the loan at 
    its sole and absolute discretion. The NFPL in its sole and absolute discretion may either sanction or reject the
    application for granting the loan. In case of rejection, the NFPL shall communicate the reason for rejection.
    (f) Informed me/us that loan application may be disposed by NFPL within 30 working days of receipt of the same subject 
    to submission of all documents and details as may be required by NFPL in processing the Loan along with the 
    requisite fees. 
    (g) The NFPL reserves its right to reject the loan application and retain the loan application form along with the
...
{ align: 'justify',  lineGap: 5 }
  ...
  .fontSize(9)
  .text(`
 (h) Informed to me/us that the NFPL shall have the right to make disclosure of any information relating to me/us including
  personal information, details in relation to loan, defaults, security, etc to the Credit Information Bureau of India 
  (CIBIL) and/or any other governmental/regulatory/statutory or private agency/entity,credit bureau, RBI, the RCPL’s other
  ...
  manner and through such medium as it may be deemed necessary by the publisher / NFPL/ RBI, including publishing the 
 ...
 (j) Informed me/us that the NFPL will send the Offer Letter to me/us on the e-mail ID mentioned by me/us in the loan application.
`,
{ align: 'justify',  lineGap: 5 }
).moveDown();
...
function drawTitletitt(sectionTitle) {
  ...
  const startY = doc.y;  
  const titleBackgroundColor = "#00BFFF";  
...
place:-${allPerameters.branchName1}
  `,
{ align: 'justify',  lineGap: 5 }
);
...
async function namDevApplicantLatter(customerId, selections = "ac" ) {
  try {
...
// console.log(customerSelections,"customerSelections"); // ["acg"]
  ...
  // console.log("BranchNameId",BranchNameId) bankDeatilsKycs
        ...
        // }
        // const newBranch = 
        // let branchName = branchData.city; 

        // let branchcity = branchData.city; 
    ...
    // Prepare parameters
     ...
     const netMonthlyIncome =finalsanctionDetails?.netCalculationNew?.overAllTotalNetMonthlyIncome;
    // console.log("netMonthlyIncome",netMonthlyIncome)
    const roundedIncome = Math.round(netMonthlyIncome);
    const allParameters = {
      branchName1:branchData?.city||"NA",
      ...
      loanAmountRequested: finalSanctionDetails?.finalLoanAmount|| "NA",
              ...
              appLandmark: applicantDetails?.houseLandMark || "NA",
              appCountry:"India",
              appResidence:applicantDetails?.residenceType || "NA",
              ...
              occupation:applicantDetails?.occupation || "NA",
              ...
              coAppvoterId : coApplicantDetails?.[0]?.voterIdNo || "NA",//page no.1 Indian
              ...
              coAppNoOfYearsATCurrentAddress:coApplicantDetails?.[0]?.noOfyearsAtCurrentAddress || "NA",
               ...
               coAppvoterId2 : coApplicantDetails?.[1]?.voterIdNo || "NA",//page no.1 Indian
               coAppreligion2:coApplicantDetails?.[1]?.religion || "NA",
              //  coAppNationality2:coApplicantDetails?.[1]?.nationality || "NA",
              coAppNationality2: "Indian",
               ...
               coAppcity2 : coApplicantDetails?.[1]?.permanentAddress?.city|| "NA",
               coAppdistrict2 : coApplicantDetails?.permanentAddress?.[1]?.district|| "NA",
               coAppPIN2 : coApplicantDetails?.[1]?.permanentAddress?.pinCode|| "NA",
               coAppState2 : coApplicantDetails?.[1]?.permanentAddress?.state|| "NA",
               ...
               coappocuupation1:coApplicantDetails?.[0]?.occupation || "NA",
            ...
            guaNationality:guarantorDetails?.nationality||"NA",
          ...
          guaVoterId : guarantorDetails?.docType === 'voterId' ? guarantorDetails?.docNo || '':'NA',
            ...
            guaUdhyam:"na",
              ...
              //   communicationAddress gua guarantorDetails?.occupation || "NA",
              ...
              appImage:applicantDetails?.applicantPhoto.trim()||"NA",
    ...
    // let pdfPath; 
    // console.log(pdfPath,"let pdfPath")
    
    // Generate PDF with skipped pages
    // if (customerSelections.includes("acg") ) {
    //   const pdfPath = await growpdf1(allParameters );
    //   console.log(pdfPath,"acg")
    ...
    //   const pdfPath = await growpdf(allParameters );
    //   console.log(pdfPath,"accg")
    ...
    //   const pdfPath = await growpdf2(allParameters );
    //   console.log(pdfPath,"acc")
    ...
    //   const pdfPath = await growpdf3( allParameters );
    //   console.log(pdfPath,"ac")
    //   // return success(res, "PDF generated successfully", pdfPath);
    //   // return  pdfPath;
     ...
     else if (selectionData === "ac")
   {
    ...
    if (!pdfPath) {
      ...
      return { error: "PDF generation failed" };
          ...
          const uploadResponse = await uploadPDFToBucket(pdfPath, `namDevApplicantConditions${Date.now()}.pdf`);
          const url = uploadResponse.url
          console.log(url,"url")        
          await finalsanctionModel.findOneAndUpdate(
            { customerId }, // Query to find the specific customer document
            {
              $set: { "naamDevPdflink.naamDevApplicantPdf": url } // Dot notation for nested update
            },
...
module.exports = { growpdf, namDevApplicantLatter };