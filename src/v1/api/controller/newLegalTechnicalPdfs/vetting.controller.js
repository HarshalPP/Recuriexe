...
// const generateSanctionLetterPdf = async(res,customerId) =>{
  const vettingPdf = async(customerId,logo,partnerName) =>{
...
// const AppAddress = applicantDetails?.fullName === internalLegalDATA?.sellerName? (applicantDetails?.localAddress?.addressLine1
...
// const AppAddress = internalLegalDATA?.sellerName  === applicantDetails?.fullName? (applicantDetails?.localAddress?.addressLine1 
//   || (internalLegalDATA?.sellerName  === coApplicantDetails?.[0]?.fullName ? coApplicantDetails?.[0]?.localAddress?.addressLine1 : 'NA')): 'NA';
    ...
    const allPerameters = {
      ...
      // appAdress:applicantDetails?.fullName === internalLegalDATA?.sellerName ? applicantDetails?.localAddress?.addressLine1 || coApplicantDetails?.[0]?.localAddress?.addressLine1:'NA',
    ...
    console.log(allPerameters,"allPerameters")
      ...
      }const uploadResponse = await uploadPDFToBucket(pdfPath, `vetting${Date.now()}.pdf`);
...
module.exports = { sanctionLetterPdf, vettingPdf };
