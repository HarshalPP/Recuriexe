...
const myEmitter = new EventEmitter();
  ...
  doc.on('end', () => console.log('PDF generated successfully!'));
    ...
    // const pdfFilename = `7pagervetting.pdf`;
...
` },
  ...
  doc.font('Helvetica')
    ...
    .text(
      `II. Original Noc Certificate No.${allPerameters.Noccertificateno} Dated ${allPerameters.NoccertificateDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName } Seal & ${allPerameters.Noc_certificateSign}.`, {align:`left`}).moveDown()
    .text(
      ...
      .text(
        ...
        .text(
          `VI. Registered Mortgage Deed No.${allPerameters.rmDeedNo} Dated ${allPerameters.RmDeedDate} Regarding Said Property With Actual Area And House No .`, {align:`left`}).moveDown(5)
  doc.font('Helvetica')
  .text(
`That in furtherance to my Title Opinion and Search Report the aforementioned document is placed before me. on perusal of the document, I find it in order and properly executed and duly stamped.`,{align:'left'})
  ...
  // const generateSanctionLetterPdf = async(res,customerId) =>{
    const sevenPagervettingPdf = async(customerId,logo,partnerName) =>{
  ...
  : coApplicantDetails?.[0]?.fullName === internalLegalDATA?.sellerName
    ...
    : 'NA';
      ...
      sealandSignedBy:internalLegalDATA?.Property_Tax_Reciept?.signedBy||"NA",
        ...
        const uploadResponse = await uploadPDFToBucket(pdfPath, `7pagervetting${Date.now()}.pdf`);
  ...
  module.exports = { sanctionLetterPdf, sevenPagervettingPdf };
