...
const myEmitter = new EventEmitter();
  ...
  async function sanctionLetterPdf(allPerameters,logo,partnerName) {
    ...
    // const pdfFilename = `finallegalPdf.pdf`;
...
.text(`This Is With Reference To Your Request For My Legal Opinion In The Matter Of The Title Of The Said Property.I Have Carefully Examined The Relevant Documents Of The Said Property Given To Me Which Are In Order. As Such I Am Submitting Here-In-Below My Legal Opinion On The Captioned Matter:-`, { align: 'left' })
    ...
    .font('Helvetica-Bold')
     ...
     .text(
      `III. Copy of Property Tax Receipt No.${allPerameters.taxReciptNo} Dated ${allPerameters.taxReciptDate} For Current Year Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour 0f ${allPerameters.sellerName} S/o ${allPerameters.sellerFatherName} Seal & Sign By ${allPerameters.sealandSignedBy}.`, {align:`left`}).moveDown();
...
doc.moveDown()
    doc.font('Helvetica')
    .text(
      `1 Documents To Be Collected By ${allPerameters.partnerName} Prior To Disbursement Of Cheque-As Per bellow  `,{align:`left`}).moveDown(0.5)
      // .text(
    ...
    doc.font('Helvetica').text(`II. Original Praman Patta No.${allPerameters.pramanPatraNo} Dated ${allPerameters.praMANpATRADate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.sellerName} S/o ${allPerameters.sellerFatherName} Seal & Sign By ${allPerameters.pramanPatraSign}. `,{align:'left'}).moveDown()
    ...
    .font('Helvetica')
    .moveDown()
    doc.font('Helvetica').font('Helvetica-Bold').text(`Documents To Be Collected By Prior To Disbursement of Cheque-${allPerameters.partnerName} `,50,170,{align:`left`,underline:true}).moveDown();
  ...
  // const generateSanctionLetterPdf = async(res,customerId) =>{
    const finallegalPdf = async(customerId,logo,partnerName) =>{
...
const formatDate = (praMANpATRADate) => {
  ...
  return `${day}-${month}-${year}`; // Final format
      ...
      const allPerameters = {
        ...
        if (!pdfPath) {
         ...
         const uploadResponse = await uploadPDFToBucket(pdfPath, `finallegalPdf${Date.now()}.pdf`);
  ...
  module.exports = { sanctionLetterPdf, finallegalPdf };
