...
const myEmitter = new EventEmitter();
  ...
  async function sanctionLetterPdf(allPerameters,logo,partnerName) {
    ...
    // const pdfFilename = `BtsevenPagerLegal.pdf`;
...
doc.moveDown(2);
doc.font(font).fillColor("black").text(`Dated: ${allPerameters.date}`, 50, 160,{align:'right'});
doc.moveDown();
...
doc.fontSize(14).font('Helvetica-Bold')
    ...
    // doc.font('Helvetica-Bold').text(`Loan Application Of :                 ${allPerameters.applicantName} & ${allPerameters.CoapplicantName}`, { align: 'left' }).moveDown(0.6);
    ...
    const loanTableData = [
      { field1: "1. Name Of The Applicant/Borrower(S)", value1: `${allPerameters.applicantName}` },
      { field1: "2. Name Of The Co-Applicant:", value1: `${allPerameters.CoapplicantName} & ${allPerameters.CoapplicantName1}` },//BT_BANK_NAME
      { field1: `3. Mortgage Loan Disbursement\n Cheque To Be Issued In Favour Of :`, value1: `${allPerameters.applicantName},S/o ${allPerameters.applicantFatherName}/${allPerameters.BT_BANK_NAME}` },
      { field1: "4. Nature Of The Property:", value1: `Freehold` },
      { field1: "5. Status Of Report :", value1: `Positive` },
      { field1: "6. Description Of The Property :", value1: `${allPerameters.techFullAdress}` },
    ];
    ...
    //     .text('Boundary Of The Plot',50, doc.y,{width: 200, align: 'left' })
    ...
    addHeader(doc, logoPath);
        ...
        doc.fontSize(10).fillColor("black").font('Helvetica-Bold')
            .text('Boundary Of The Plot',50, 168,{width: 200, align: 'left' })
    ...
    .fillColor("black")
    .font('Helvetica-Bold')
        .text('7. Copy Of Documents\n Examined:',50, 310, {align:`left`}).moveDown();

        doc.fontSize(11)
        .font('Helvetica')
            .text(`I.Copy Gram Panchayat No${allPerameters.gramPanchayatNo} Dated ${allPerameters.gramPanchayatDate} Issued By ${allPerameters.gramPanchayatSign} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.gramPanchayatSign}.`, {align:`left`}).moveDown()
            .text( `II.Copy Noc Certificate No.${allPerameters.Noccertificateno} Dated ${allPerameters.NoccertificateDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName }Seal & Sign By ${allPerameters.Noc_certificateSign}.`, {align:`left`}).moveDown()
            .text( `III.Copy Building Permission Certificate No. ${allPerameters.BuildingPermissionNo} Dated ${allPerameters.BuildingPermissionDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} & Sign By ${allPerameters.Buliding_Permission_CertificateSign}.`, {align:`left`}).moveDown()
            .text( `IV.Copy Mutation Certificate No. ${allPerameters.mutationNo} Dated ${allPerameters.mutationDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.Mutation_CertificateSign}.`, {align:`left`}).moveDown()
            .text(`V.Copy Owner Certificate No. ${allPerameters.ownershipNo} Dated ${allPerameters.ownershipDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.Owner_CertificateSign}.`, {align:`left`}).moveDown()
            .text(`VI.Property Tax Receipt No.${allPerameters.Property_Tax_RecieptNo} Dated ${allPerameters.Property_Tax_RecieptDate} For Current Year Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.sealandSignedBy}.`, {align:`left`}).moveDown()
            .text(`VII.LOD of ${allPerameters.BT_BANK_NAME}.`, {align:`left`}).moveDown()


   

    // doc.font('Helvetica-Bold')
    // .text('8. Observations',{align:'left'});
    // y += 20;
    ...
    // .text(
    //   `${allPerameters.techFullAdress} Owner And Possessed By Mr.${allPerameters.applicantName} S/o ${allPerameters.applicantFatherName}.`, {align:`left`}).moveDown()
    // .text(
    //   `I.That Gram Panchayat Issued Certificate No.Q/P/2023 Dated 14-09-2023 Issued By Sarpanch & Sachiv Tehsil Malhargarh District Mandsaur Madhya Pradesh In Favour Of Balram S/O Karulal Seal & Sign By Sarpanch & Sachiv.`, {align:`left`}).moveDown();
    ...
    doc.font('Helvetica-Bold')
    .text('8. Observations',50,168,{align:'left'});
    y += 20;
    doc.text('That As Per Available Copy of Documents,Instructions And Policy of The Company We Found That –',{align:'left'});
      doc.moveDown()
    doc.font('Helvetica')
    ...
    .text(
      `${allPerameters.techFullAdress} Owner And Possessed By Mr.${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName}.`, {align:`left`}).moveDown()
    // .text('9. Conclusion', {align:`left`});
    // doc.moveDown()
    // y += 20;
    // doc.font('Helvetica').text(`
    ...
    doc.font('Helvetica')
    .text(
        `I.That Gram Panchayat Issued Certificate No. ${allPerameters.gramPanchayatNo} Dated ${allPerameters.gramPanchayatDate} Issued By  ${allPerameters.gramPanchayatSign}  Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By  ${allPerameters.gramPanchayatSign}.`,{align:`left`}).moveDown()
    .text(
      `II.That Also Noc Certificate No.${allPerameters.Noccertificateno} Dated ${allPerameters.NoccertificateDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.Noc_certificateSign}.`,{align:`left`}).moveDown()       
    .text(
      `III.Building Permission Certificate No. ${allPerameters.BuildingPermissionNo} Dated ${allPerameters.BuildingPermissionDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.Buliding_Permission_CertificateSign}.`,{align:`left`}).moveDown()
    .text(
      `IV.Said Owner/Possession Holder Having Mutation Certificate No. ${allPerameters.mutationNo} Dated ${allPerameters.mutationDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.Mutation_CertificateSign}.`, {align:`left`}).moveDown()
    .text(
      `V.Said Owner /Possession Holder Also Paid Property Tax Receipt No.${allPerameters.Property_Tax_RecieptNo} Dated ${allPerameters.Property_Tax_RecieptDate} For Current Year Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour Of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.sealandSignedBy}.`, {align:`left`}).moveDown()
      .text(
        `VI. Said Property Already Mortage With ${allPerameters.BT_BANK_NAME}.`, {align:`left`}).moveDown();
    ...
    y += 20;
    doc.font('Helvetica').text(
    `${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Is The Recorded Owner Of Freehold Residential Land Admeasuring Area ${allPerameters.landArea} Sq Ft ${allPerameters.techFullAdress} Having A Clear And Marketable Title Over The Said Property As Per Available Record And Documents.`, {align: 'left' }).moveDown();
   
   
  //     doc
  //   .fontSize(11)
  //   .font('Helvetica-Bold').text('Search Certificate:',{align:`left`}).moveDown();
  //   y += 20;
  //   doc.font('Helvetica')
  //   .text(
  ...
  .text(
    `As Desired, The Documents As Received, Are Returned Herewith. We Confirm That The Subject Property/Floor Is Covered Under The Sarfaesi Act.After Takingnoc From Competent Authority For Mortgage ,Anaffidevit Regarding Ownership And Possession Along With Property Free From Litigation Or Disputed Or Free From Lien or Mortgage Must Be Taken As No Manual Record Since 2015 Available Also Some Record Not Found or Turn In Sro.`,{align:`left`}).moveDown()
    .font('Helvetica-Bold').text('Encl:-Electro State Copy Of Documents Examined As Per Para 7 (I To Vi)',{align:`left`}).moveDown()
    .font('Helvetica-Bold').text('Documents Required For Creation Of An Equitable Mortgage On The Said Property',{align:`left`,underline:true}).moveDown();
    doc
    .fontSize(11)
    ...
    doc.font('Helvetica')
    .text(
    `1.Documents To Be Collected By ${allPerameters.BT_BANK_NAME} Prior To Disbursement Of Cheque-As Per Annexure-I`,{align:`left`})
    .text(
    `2. Documents Required Post Disbursement/Follow-Up-As Per Annexure-I`,{align:`left`}).moveDown()
      .font('Helvetica-Bold').text(`Documents To Be Collected By Prior To Disbursement Of Cheque-${allPerameters.BT_BANK_NAME}`,{align:`left`,underline:true}).moveDown();
    ...
    doc.font('Helvetica')
    .text(
      `I. Original Gram Panchayat Patta No. ${allPerameters.gramPanchayatNo} Dated ${allPerameters.gramPanchayatDate} Issued By Sarpanch & Sachiv Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour Of ${allPerameters.propertyOwnerName} S/O ${allPerameters.propertyOwnerFatherName} Seal & Sign By  ${allPerameters.gramPanchayatSign}.`,{align:`left`}).moveDown()       
    .text(
      `II. Original Noc Certificate No.${allPerameters.Noccertificateno} Dated ${allPerameters.NoccertificateDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour Of ${allPerameters.propertyOwnerName} S/O ${allPerameters.propertyOwnerFatherName }Seal & Sign By ${allPerameters.Noc_certificateSign}.`, {align:`left`}).moveDown()
    .text(
      `III. Original Building Permission Certificate No. ${allPerameters.BuildingPermissionNo} Dated ${allPerameters.BuildingPermissionDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour Of ${allPerameters.propertyOwnerName} S/O ${allPerameters.propertyOwnerFatherName} & Sign By ${allPerameters.Buliding_Permission_CertificateSign}.`, {align:`left`}).moveDown()
    .text(
      `IV.Original Mutation Certificate No. ${allPerameters.mutationNo} Dated ${allPerameters.mutationDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour Of ${allPerameters.propertyOwnerName} S/O ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.Mutation_CertificateSign}.`, {align:`left`}).moveDown()
      ...
      .font('Helvetica')
      .text(
        `V.Original Ownership Certificate No. ${allPerameters.ownershipNo} Dated ${allPerameters.ownershipDate} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour Of ${allPerameters.propertyOwnerName} S/O ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.Owner_CertificateSign}.`,50,168,{align:`left`}).moveDown()
      .text(
        `VI. Property Tax Receipt No. ${allPerameters.Property_Tax_RecieptNo} Dated ${allPerameters.Property_Tax_RecieptDate} For Current Year Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour Of ${allPerameters.propertyOwnerName} S/O ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.sealandSignedBy}.`, {align:`left`}).moveDown()
      .text(
        `VI. Registered Mortgage Deed Regarding Said Property With Actual Area And House No .`, {align:`left`}).moveDown()
        .text(
            `VIII. LOD and NOC OF ${allPerameters.BT_BANK_NAME}.`, {align:`left`}).moveDown(5)
  ...
  // const generateSanctionLetterPdf = async(res,customerId) =>{
    const BtsevenPagerLegalpdf = async(customerId,logo,partnerName) =>{
  ...
  return `${day}-${month}-${year}`; // Final format
      ...
      const allPerameters = {
        coOwnerShipSign:internalLegalDATA?.co_ownership_deed?.signedBy||"NA",
        EmDeedSign:internalLegalDATA?.EM_DEED?.signedBy||"NA",
        pramanPatraSign:internalLegalDATA?.pramanPatra?.signedBy||"NA",
        RmDeedSign:internalLegalDATA?.RM_DEED?.signedBy||"NA",
        gramPanchayatSign:internalLegalDATA?.gramPanchayat?.signedBy||"NA",
        Noc_certificateSign:internalLegalDATA?.Noc_certificate?.signedBy||"NA",
        Buliding_Permission_CertificateSign:internalLegalDATA?.Buliding_Permission_Certificate?.signedBy||"NA",
        Mutation_CertificateSign:internalLegalDATA?.Mutation_Certificate?.signedBy||"NA",
        Owner_CertificateSign:internalLegalDATA?.Owner_Certificate?.signedBy||"NA",
        ...
        sealandSignedBy:internalLegalDATA?.Property_Tax_Reciept?.signedBy||"NA",
        ...
        appAdress:applicantDetails?.localAddress?.addressLine1||"NA",
        ...
        Property_Tax_RecieptNo:internalLegalDATA?.Property_Tax_Reciept?.no||"NA",
        Property_Tax_RecieptDate:formatDate(internalLegalDATA?.Property_Tax_Reciept?.date)||"NA",
        ...
        Noccertificateno:internalLegalDATA?.Noc_certificate?.no||"NA",
        NoccertificateDate:formatDate(internalLegalDATA?.Noc_certificate?.date)||"NA",

        BuildingPermissionNo:internalLegalDATA?.Buliding_Permission_Certificate?.no||"NA",
        BuildingPermissionDate:formatDate(internalLegalDATA?.Buliding_Permission_Certificate?.date)||"NA",

        mutationNo:internalLegalDATA?.Mutation_Certificate?.no||"NA",
        mutationDate:formatDate(internalLegalDATA?.Mutation_Certificate?.date)||"NA",

        ownershipNo:internalLegalDATA?.Owner_Certificate?.no||"NA",
        ownershipDate:formatDate(internalLegalDATA?.Owner_Certificate?.date)||"NA",
        ...
        const uploadResponse = await uploadPDFToBucket(pdfPath, `BtsevenPagerLegal${Date.now()}.pdf`);
  ...
  module.exports = { sanctionLetterPdf, BtsevenPagerLegalpdf };
