...
const internalLegalModel = require("../../model/finalApproval/internalLegal.model.js"); 
  ...
  async function sanctionLetterPdf(allPerameters,logo,partnerName) {
    ...
    // const pdfFilename = `Executedvetting.pdf`;
  ...
  // const generateSanctionLetterPdf = async(res,customerId) =>{
    const executedvettingPdf = async(customerId,logo,partnerName) =>{
  ...
  : coApplicantDetails?.[0]?.fullName === internalLegalDATA?.sellerName
    ...
    : 'NA';
      ...
      sealandSignedBy:internalLegalDATA?.taxReceipt?.signedBy||"NA",
        ...
        const uploadResponse = await uploadPDFToBucket(pdfPath, `Executedvetting${Date.now()}.pdf`);
  ...
  module.exports = { sanctionLetterPdf, executedvettingPdf };
