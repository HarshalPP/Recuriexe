const {
  ...
  async function sanctionLetterPdf(allPerameters,logo,partnerName) {
    ...
    // const pdfFilename = `ExecutedfinallegalPdf.pdf`;
...
doc.moveDown()
  ...
  // const generateSanctionLetterPdf = async(res,customerId) =>{
    const executedfinallegalPdf = async(customerId,logo,partnerName) =>{
...
const formatDate = (praMANpATRADate) => {
  ...
  return `${day}-${month}-${year}`; // Final format
      ...
      const allPerameters = {
        ...
        const uploadResponse = await uploadPDFToBucket(pdfPath, `ExecutedfinallegalPdf${Date.now()}.pdf`);
  ...
  module.exports = { sanctionLetterPdf, executedfinallegalPdf };
