...
const myEmitter = new EventEmitter();
  ...
  async function sanctionLetterPdf(allPerameters,logo,partnerName) {
    ...
    // const pdfFilename = `bt7PagerVetting.pdf`;
  ...
  doc.fontSize(14).font('Helvetica-Bold')
        ...
        .text(`CIN-${allPerameters.partnerCinNo}`, { align: 'left' }).moveDown();
  
    doc.fontSize(18).font('Helvetica-Bold').text('CERTIFICATE',{align:'center',underline:true}).moveDown(0.6);
    
    doc.fontSize(10).text(`Loan A / c:`,{align:'left',continued:true});
  doc.fontSize(10).text(`Mr ${allPerameters.applicantName} , Mrs ${allPerameters.CoapplicantName}`,{align:'right'});
  ...
  //         { field1: "Name, Father’s name & Address of the present owner", field2: ": ", value: `${allPerameters.sellerName} S/o ${allPerameters.sellerFatherName} \n\n ${allPerameters.appAdress}`},
  //         { field1: "Description of the Property", field2: ": ", value: `${allPerameters.techFullAdress},
  // \n\n\n\n\n
  // Boundries :-
  ...
  function BoundariesFunction(tableData) {
    ...
    tableData.forEach((row) => {
      ...
      doc.moveDown()
      const boundariesData = [
          {field1:"1.", field2: "Name of the Property Owner", field3: ": ", field4: `${allPerameters.propertyOwnerName} & ${allPerameters.propertyOwnerFatherName}`},
          ...
          {field1:"4.", field2: "Name, Father’s name & Address of the present owner", field3: ": ", field4: `${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} \n\n ${allPerameters.appAdress}`},
  ...
  ` },
    ...
    doc.font('Helvetica')
    .text(
      `I. ORIGINAL GRAM PANCHAYAT PATTA NO. ${allPerameters.gramPanchayatNo} Dated ${allPerameters.gramPanchayatDate} Issued By  ${allPerameters.gramPanchayatSign} Gram Panchyat ${allPerameters.gramPanchayat} Tehsil  ${allPerameters.Tehsil} District  ${allPerameters.district} State  ${allPerameters.state} In Favour of ${allPerameters.propertyOwnerName} S/o ${allPerameters.propertyOwnerFatherName} Seal & Sign By ${allPerameters.gramPanchayatSign}.`,50, 168,{align:'left'}).moveDown()
            doc.font('Helvetica')
        .text(
      ...
      .text(
          ...
          .text(
              ...
              .text(
                  `VII. REGISTERED MORTGAGE DEED NO.  ${allPerameters.rmDeedNo} Dated ${allPerameters.RmDeedDate} Regarding Said Property With Actual Area And House No .`,{align:'left'}).moveDown()
                  .text(
                    `VIII. LOD and NOC OF ${allPerameters.BT_BANK_NAME}.`, {align:`left`}).moveDown()
    doc.font('Helvetica')
    .text(
  ...
  // const generateSanctionLetterPdf = async(res,customerId) =>{
    const bt7PagervettingPdf = async(customerId,logo,partnerName) =>{
  ...
  : coApplicantDetails?.[0]?.fullName === internalLegalDATA?.sellerName
    ...
    : 'NA';
      ...
      sealandSignedBy:internalLegalDATA?.Property_Tax_Reciept?.signedBy||"NA",
        ...
        rmDeedNo:internalLegalDATA?.RM_DEED?.no||"NA",
        RmDeedDate:formatDate(internalLegalDATA?.RM_DEED?.date)||"NA",
        ...
        const uploadResponse = await uploadPDFToBucket(pdfPath, `bt7PagerVetting${Date.now()}.pdf`);
  ...
  module.exports = { sanctionLetterPdf, bt7PagervettingPdf };
