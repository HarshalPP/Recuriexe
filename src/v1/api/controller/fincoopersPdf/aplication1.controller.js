...
function addLogo(doc) {
  if (fs.existsSync(logo)) {
    doc.image(logo, 400, 50, { fit: [150, 50], align: "left", valign: "bottom" });
  } else {
...
*Requirement of documents might vary according to the scheme chosen.`, startX + padding, currentY + padding, { align: 'left' });
  ...
  const loanDetails = [
    { key: "Loan Amount Requested", value: allPerameters.loanAmount || `${allPerameters.loanAmountRequestedd}` },
...
async function FincooperApplicantionPdf(customerId ,logo,partnerName, ) {
  try {
      ...
      let allParameters = {
        ...
        customerNO:sanctionPendencyDetails?.partnerCustomerId|| "NA",
           loanAmountRequested: customerDetails?.loanAmount || "NA",// page no 1
            ...
            sourceType : "NA",
            loanAmountRequestedd:finalsanctionDetails?.loanAmount|| "NA",// page no 1
     ...
     else if (selectionData === "ac")
   {
    ...
    if (!pdfPath) {
      ...
      return { error: "PDF generation failed" };
          ...
          const uploadResponse = await uploadPDFToBucket(pdfPath, `fincooperApplicantPdf${Date.now()}.pdf`);
...
module.exports = {FincooperApplicantionPdf };
