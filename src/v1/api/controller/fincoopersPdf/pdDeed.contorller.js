...
doc.fontSize(12).text('FinCoopers Capital Pvt Ltd', { align: 'center' });
...
doc.font(font).text(
  `FinCoopers Capital Pvt Ltd, a company incorporated under the Companies Act, 1956 with \ncorporate identity number 67120MP1994PTC008686, registered as a non-banking financial \nservices company with in the meaning of the Reserve Bank of India ("RBI") Act, 1934 and having \nits registered office at 174/3 Nehru Nagar, Indore - 452011 (M.P.) (here in after referred to as the.`,
  { align: 'left', indent: 40,lineGap: 5 ,
  }
)
.
...
.fontSize(8)
.font('Helvetica')
.text(`Fin Coopers Capital Pvt Ltd`,
  {  align: 'left', indent: 40 })
doc.moveDown(0.5);
...
doc.moveDown();
doc.font(font).text(`For ; ${allPerameters.customerName},`, { align: 'left', indent: 40 });
...
async function FincooperPgDeedPdf(customerId, logo,partnerName) {
  try {
...
const formatDate = (praMANpATRADate) => {
  ...
  return `${day}-${month}-${year}`; // Final format
    ...
    const allPerameters = {
      ...
      lenderName:"NA",
      place: extnalvenderDetails?.branchNameId?.address || "NA",
      day: day || "NA",
      month: month || "NA",
      year: year || "NA",
    ...
    const uploadResponse = await uploadPDFToBucket(pdfPath, `fincooperPgDeedPdf${Date.now()}.pdf`);
       ...
       const tt= await finalsanctionModel.findOneAndUpdate(
          ...
          {
            $set: { "fcplPdfUrls.fincooperPgDeedPdf": url } // Dot notation for nested update
          },
...
module.exports = { PgDeedLetterPdf, FincooperPgDeedPdf  };
