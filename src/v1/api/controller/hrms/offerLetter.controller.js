...
async function generateOfferLetter3(
  candidateDetails,
  position,
  package,
  joiningDate,
  ...
  doc.moveDown(0.5);
  doc.text(`${candidateDetails.preferredLocation},`, { align: 'left' });
  doc.moveDown(0.5);
  doc.text(`${candidateDetails.state}`, { align: 'left' });
  doc.moveDown(2);
...
if (isInternship) {
  ...
  doc.text(`We are pleased to offer you an Internship position as a `, { continued: true });
  boldText(doc, position);
  doc.text(` at `, { continued: true });
  boldText(doc, company);
  doc.text(`. Please find below the terms and conditions related to your internship with us.`);
  doc.moveDown(1);

  doc.text(`In summary:`);
  doc.moveDown(0.8);

// Helper function to bold key and normal value on a single line
function keyValueLine(key, value) {
  doc.font(fontBold).fontSize(fontBoldSize).text(key, { continued: true });
  doc.font(font).fontSize(fontSize).text(value);
}

// Render each line
keyValueLine('Internship Start Date: ', moment(joiningDate).format('MMMM Do, YYYY'));
keyValueLine('Internship Stipend (Monthly): INR ', Number(package).toLocaleString("en-IN", { minimumFractionDigits: 2 }));
keyValueLine('Stipend Frequency: ', 'Monthly, deposited directly to your bank account');
keyValueLine('Duration: ', '3 months (probationary/evaluation period)');
keyValueLine('Work Schedule: ', 'Monday to Saturday, 10:00 AM to 7:00 PM');
...
doc.moveDown(2);
  ...
  doc.text(`This internship is designed to provide you with hands-on experience in a real-world working environment. During this period, your performance and contributions will be evaluated to determine further opportunities within the organization.`);
  doc.moveDown(1);

  doc.text(`If you have any questions or require further information, please feel free to contact the HR Department.`);
  doc.moveDown(1);

  doc.text(`We look forward to welcoming you to our team and supporting your professional development during your time with us.`);
}
    ...
    doc.text(` and for the six month of your joining you will be serving an evaluation/probation period.`);
    doc.moveDown(1);
  ...
  await uploadToSpaces(bucketName, pdfFilename, pdfBuffer, 'public-read', 'application/pdf');
...
async function generateOfferLetterPDF1(
  candidateDetails,
  position,
  package,
  joiningDate,
...
module.exports = { offerLetterPDF  , generateOfferLetterPDF , generateAppointmentPDF,generateOfferLetterPDF1 , generateOfferLetter3};
