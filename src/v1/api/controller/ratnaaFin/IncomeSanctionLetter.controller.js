  ...
  async function ratannaFinSanctionLetterPdf(allPerameters) {
    ...
    function drawTable(tableData) {
      ...
      tableData.forEach((row, rowIndex) => {
        ...
        if (isSpecialRow) {
          // doc
          //   .fillColor("#0070C0 ") // Background color
          //   .rect(startX, startY, columnWidths[0], rowHeight)
          //   .fill()
          //   .stroke("black", 1); // Thin border
...
doc
  ...
  .stroke();
    ...
    const loanTableData = [
      ...
      { field1: "Description of Collateral Property", value1: `${allPerameters.AddressDetails}` },
       ...
       function drawTable1(tableData) {
            ...
            tableData.forEach((row, rowIndex) => {
              ...
              if (isChargesRow) {
                ...
                // doc
                //   .fillColor("#0070C0") // Blue background
                //   .rect(startX, startY, columnWidths[0], rowHeight)
                //   .fill()
...
doc
          ...
          const loanSecondTable = [
              ...
              field1: "CHARGES" },
            {
              field1:
                "Login Fees",
              value1:
                `Rs.${allPerameters.loginFees} /- (Inclusive of Applicable Taxes)`,
            },
            {
              field1:
  ...
  //         if (isSpecialRow) {
  //             doc
  //                 .fillColor("#0070C0") // Background color for "S. No" rows
  ...
  function latterTableFunction(tableData) {
    ...
    let startY = doc.y + 10;
    const totalWidth = 500; // Total column width
    const keyWidth = Math.round(totalWidth * 0.20); // 20% of the total width
    const valueWidth = totalWidth - keyWidth; // Remaining width for the value column
    ...
    tableData.forEach((row, rowIndex) => {
        ...
        const field1TextHeight = doc.font(font).fontSize(7.2).heightOfString(row.field1, { width: keyWidth, wordBreak: 'break-word' });
        let value1TextHeight = row.value1 ? doc.font(font).fontSize(7.2).heightOfString(row.value1, { width: valueWidth, wordBreak: 'break-word' }) : 0;
        ...
        // Draw background color for special row
        if (isSpecialRow) {
            doc.fillColor("#0070C0").rect(startX, startY, totalWidth, rowHeight).fill();
        } else {
            doc.fillColor(rowIndex % 2 === 0 ? "#f5f5f5" : "#ffffff").rect(startX, startY, totalWidth, rowHeight).fill();
        }

        // Draw text in the key column (field1)
        doc.font(font).fillColor(isSpecialRow ? "white" : "black").fontSize(7.2).text(row.field1, startX + 5, startY + 5, {
            baseline: "hanging",
            width: keyWidth,
            height: rowHeight - 10,
            align: "left",
        ...
        // Draw text in the value column (value1)
        doc.font(font).fillColor(isSpecialRow ? "white" : "black").fontSize(7.2).text(row.value1 || "", startX + keyWidth + 5, startY + 5, {
            baseline: "hanging",
            width: valueWidth,
            height: rowHeight - 10,
            align: "left",
        ...
        // Border for the key column (left)
        doc.rect(startX, startY, keyWidth, rowHeight).stroke();

        // Border for the value column (right)
        doc.rect(startX + keyWidth, startY, valueWidth, rowHeight).stroke();
    ...
    const  PreDisbursementTablee = [
      ...
      { field1: "1", value1: `Rcu/Fcu Report to be Positive` },
      { field1: "2", value1: `Legal And Technical Report to be positive` },
      { field1: "3", value1: `5 PDCs of borrower and 2 PDC of Financial Guarantor / Co-applicant to be submitted at the time of disbursement` },
      { field1: "4", value1: `Original Property documents to be summitted at the time of disbursement` },
      { field1: "5", value1: `Registered mortgage deed/Equitable Mortgage Deed to be submitted at the time of disbursement.` },
      { field1: "6", value1: `All kyc required with self attestation and OSV to be submitted before disbursement.` },
      { field1: "7", value1: `All Physical Documents to be self attested by the applicant and to be submitted before disbursement.` },
      { field1: "8", value1: `E-Nach to be done at the time of disbursement.` },
      { field1: "9", value1: `Form 60 of those Pancard is not available to be submitted before disbursement.` },
  ...
  //   console.log(customerId,"in sanction latter")
     ...
     console.log("loanPurpose",loanPurpose)
        ...
        const allPerameters = {
          ...
          loginFees: customerDetails?.loginFees,//page 2
          nonRefundableProcessingFee:"2% of loan amounts + Applicable taxes",
          documentationCharges:"If Applicable (2% of loan amounts + Applicable taxes ( For under construction cases 3% of loan amounts + Applicable taxes)",
          stampDutyCharges: "As applicable and to be borne by the Customer",
          lifeInsurancePremiumForIndividual:"If Life Insurance is Applicable then premium is to be brone by customer",
          insurancePremiumForCollateralSecurity:`If Collateral Insurance is Applicable then premium is to be brone by customer`,
