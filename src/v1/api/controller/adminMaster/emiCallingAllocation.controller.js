const {
  ...
  const emiCallingAllocationModel = require("../../model/adminMaster/emiCallingAllocation.model")
...
async function emiCallingAllocationAdd(req, res) {
  try {
    ...
    const operations = customerFinIds.map(async (customerFinId) => {
      ...
      let existingAllocation = await emiCallingAllocationModel.findOne({ customerFinId });
      ...
      if (existingAllocation) {
        ...
        const newAllocation = await emiCallingAllocationModel.create({
          customerFinId,
...
async function emiCallingAllocationUpdate(req, res) {
  try {
    // Validate request
    ...
    if (employeeId && customerFinId) {
      ...
      const updatedAllocation = await emiCallingAllocationModel.findOneAndUpdate(
        { 
          employeeId,
          customerFinId 
        },
    ...
    if (oldAllocationId && newAllocationId) {
      ...
      const updateResult = await emiCallingAllocationModel.updateMany(
        {
          $or: [
      ...
      const updatedRecords = await emiCallingAllocationModel.find({
        $or: [
...
async function emiCallingAllocationModelAndNoAllocation(req, res) {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    if (status === "allocation") {
      const employeeAllocations = await emiCallingAllocationModel.find({});
...
async function getemiCallingAllocationList(req, res) {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
    ...
    // Step 1: Fetch all data from emiCallingAllocationModel
    const employeeAllocations = await emiCallingAllocationModel.find({});
