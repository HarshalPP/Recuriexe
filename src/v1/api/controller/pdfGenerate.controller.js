...
// Function to fetch and compress image from URL
// async function fetchImageBuffer(imageUrl) {
//   try {
//     const response = await axios.get(imageUrl, { responseType: 'arraybuffer' });
...
//       .resize({ width: 800, fit: 'inside', withoutEnlargement: true }) // Resize if larger than 800px width
//       .jpeg({ quality: 70, progressive: true }) // Convert to progressive JPEG with 70% quality
//       .toBuffer();
//   } catch (error) {
...
// Updated function to preserve transparency in images
async function fetchImageBuffer(imageUrl) {
  try {
    const response = await axios.get(imageUrl, { responseType: 'arraybuffer' });
    const imageBuffer = Buffer.from(response.data, 'binary');
...
async function generateCreditPdPdf(res, customerId) {
  try {
    ...
    // Logo
    // const pdfLogo = `https://cdn.fincooper.in/STAGE/LOS/IMAGE/1738218253567_FINCOOPERSLOGO.png`;
    // if (fs.existsSync(pdfLogo)) {
    //   doc.image(pdfLogo, xCenter, 50, { width: imageWidth });
    // }
...
//  doc.image(imageBuffer, xCenter, 50, { width: imageWidth });
//  } catch (error) {
...
await addLogoToPDF(doc, pdfLogo, xCenter, imageWidth);
    ...
    // Header
    doc.moveDown(2.3);
    doc.fontSize(15).font(fontBold).text(`CUSTOMER INTERNAL VERIFICATION REPORT`, { align: 'center' });
    doc.moveDown(0.5);
...
//     const pageHeight = doc.page.height;
    ...
    const remarksByPDPerson = [];

    // Populate the loan requirement details array
    remarksByPDPerson.push(
    ...
    addSection('Remarks by PD Person', remarksByPDPerson);
    ...
    addSection('Pd Done', employeeDetail);
    ...
    //   data.incomeSource.forEach((source) => {
    //     if (source.agricultureBusiness?.agriculturePhotos) {
    //       source.agricultureBusiness.agriculturePhotos.forEach((photo) => {
    //         if (photo) {
    //           const imageUrl = photo.startsWith('http') ? photo : `${cdnBaseUrl}${photo}`;
    //           imageCollections.agricultureImages.push({ imageUrl });
    //         }
    //       });
    //     }

    //     if (source.milkBusiness?.milkPhotos) {
    //       source.milkBusiness.milkPhotos.forEach((photo) => {
    //         if (photo) {
    //           const imageUrl = photo.startsWith('http') ? photo : `${cdnBaseUrl}${photo}`;
    //           imageCollections.milkImages.push({ imageUrl });
    //         }
    //       });
    //     }

    //     // Process other image types similarly...
    //   });

    //   if (data.familyMemberPhotos) {
    //     data.familyMemberPhotos.forEach((photo) => {
    //       if (photo) {
    //         const imageUrl = photo.startsWith('http') ? photo : `${cdnBaseUrl}${photo}`;
    //         imageCollections.familyMemberImages.push({ imageUrl });
    //       }
    //     });
...
// const imageHeight = 80; // Reduced from 100
    ...
    //   doc.fontSize(12).text('All Images', { align: 'center' });
    ...
    await addImagesWithHeading(doc, 'agriculture Land Image' , agricultureLandImages, 3, 70);
...
module.exports = {generateCreditPdPdf};